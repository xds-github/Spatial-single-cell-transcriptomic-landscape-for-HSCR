library(MASS)
library(reshape2)
count_matrix <- read.csv('J:/scanpy_out/final_ann/celltype_count_total_raw.csv')
...
count_matrix <- read.csv('J:/ST-analysis/cluster_distr_processed.csv')
count_matrix <- melt(count_matrix)
cond_1 <- "HA"
cond_2 <- "HG"
cond_3 <- "CT"
...
pv = cbind.data.frame(pv_cond_2vs1, pv_cond_3vs1,pv_cond_3vs2) # !
rownames(pv) = levels(count_matrix$variable)
write.csv(pv, file = "J:/scanpy_out/final_ann/pv_celltount_immune.csv")
